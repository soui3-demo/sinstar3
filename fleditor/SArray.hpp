...
public:
	...
	int SortInsert(T newElement,BOOL bRepeat=TRUE,FUNCMP funCmp=NULL,LPARAM lParam=0,UINT sfType=SFT_ANY)
	{
		...
		int nInsertPos=-1;
		int nPos=_SortFind(&newElement,0,m_nCount,funCmp,lParam,sfType,&nInsertPos);
		if(nPos!=-1)
		{
			if(!bRepeat) return -1;
	...
	int SortFind(T *pData,FUNCMP funCmp=NULL,LPARAM lParam=0,int nBegin=0,UINT sfType=SFT_ANY)
	{
		if(nBegin>=(int)m_nCount)
			...
			return _SortFind(pData,nBegin,m_nCount,funCmp,lParam,sfType,NULL);
	}
...
protected:
	...
	//************************************************
	int _SortFind(T *pData,int nBegin,int nEnd,FUNCMP funCmp,LPARAM lParam,UINT sfType,int *pInsertPos)
	{
		ASSERT(nBegin<=nEnd);
		...
		if(nCmp>0)
			return _SortFind(pData,nMiddle+1,nEnd,funCmp,lParam,sfType,pInsertPos);
		else
			return _SortFind(pData,nBegin,nMiddle,funCmp,lParam,sfType,pInsertPos);
	}
