...
/* Basic MD5 step. Transform buf based on input.
 */
static void Transform (UINT4 *buf,UINT4 *input)
{
...
void MD5Print (MD5_CTX *mdContext,char szBuf[33])
{
	int i;
	for (i = 0; i < 16; i++)
		sprintf (szBuf+2*i,"%02x", mdContext->digest[i]);
}

void MD5String (const char *inString,char szOutput[33])
{
  MD5_CTX mdContext;
  unsigned int len = strlen (inString);

  MD5Init (&mdContext);
  MD5Update (&mdContext, (LPBYTE)inString, len);
  MD5Final (&mdContext);
  MD5Print (&mdContext,szOutput);
}
...
/* Computes the message digest for a specified file.
   ...
   return.
 */
BOOL MD5File (const char *filename,char szOutput[33],int nSegLen)
{
  ...
  pdata=(unsigned char*)malloc(nSegLen);
  MD5Init (&mdContext);
  while ((bytes = fread (pdata, 1, nSegLen, inFile)) != 0)
  {
	  MD5Update (&mdContext, pdata, bytes);
	  fseek(inFile,nSegLen,SEEK_CUR);
  }
  MD5Final (&mdContext);
  MD5Print (&mdContext,szOutput);
  fclose (inFile);
